...
class ReplayBuffer:
    ...
    def __init__(self, capacity: int, state_dim: int, action_dim: int, device: torch.device):
        ...
        self.next_masks = torch.zeros((capacity, action_dim), dtype=torch.float32)
        self.dones = torch.zeros(capacity, dtype=torch.float32)
        
        self.position = 0
        self.size = 0

    def add(self, state: np.ndarray, action: int, reward: float, next_state: np.ndarray, next_mask: np.ndarray, done: bool):
        """
        ...
        self.next_masks[self.position] = torch.from_numpy(next_mask).float()
    ...
    def sample(self, batch_size: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        return (
            ...
            self.next_masks[indices].to(self.device).detach(),
