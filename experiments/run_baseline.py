...
# (Removed legacy test network injection)


def run_experiment(num_episodes: int = 100):
    print(f"Starting Baseline Stress Test: 100 Episodes (Greedy vs Greedy)")
    ...
    for ep in range(num_episodes):
        ...
        defender = GreedyDefender("def_greedy")
        
        # Initial observation after setup
        obs = {
            "atk_greedy": env.get_observation_by_id("atk_greedy"),
            "def_greedy": env.get_observation_by_id("def_greedy")
        }
        ...
        try:
            ca_priv = env._state.vulnerability_registry.get_privilege("data")
            print(f"End Episode {ep} | Steps: {steps} | Data Priv: {ca_priv.name} | Info: {info}")
